...
Handle<Value> NodePlayer::playAndSeek(const Arguments& args) {
  HandleScope scope;
  if(args.Length() < 2) {
    return scope.Close(V8_EXCEPTION("playAndSeek needs a track as its first argument, position in ms as second."));
  }
  NodePlayer* nodePlayer = node::ObjectWrap::Unwrap<NodePlayer>(args.This());
  NodeTrack* nodeTrack = node::ObjectWrap::Unwrap<NodeTrack>(args[0]->ToObject());
  int seekToPosition = args[1]->ToInteger()->Value();

  try {
    nodePlayer->player->playAndSeek(nodeTrack->track, seekToPosition);
  } catch (const TrackNotPlayableException& e) {
    return scope.Close(V8_EXCEPTION("Track not playable"));
  }
  return scope.Close(Undefined());
...
void NodePlayer::init() {
  ...
  NODE_SET_PROTOTYPE_METHOD(constructorTemplate, "playAndSeek", playAndSeek);
